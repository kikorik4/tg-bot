...
RUSSIAN_LANGUAGE = Language(days=("Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"), months=(
...
class Calendar:
    ...
    def create_calendar(self, name: str = "calendar_1", year: int = None, month: int = None, ) -> InlineKeyboardMarkup:
        ...
        keyboard.add(InlineKeyboardButton("Предыдущий месяц",
                                          callback_data=calendar_callback.new("PREVIOUS-MONTH", year, month, "!")),
                     InlineKeyboardButton("Следующий месяц",
    ...
    def create_months_calendar(self, name: str = "calendar", year: int = None) -> InlineKeyboardMarkup:
        ...
        for i, month in enumerate(zip(self.__lang.months[0::2], self.__lang.months[1::2])):
            keyboard.add(
                InlineKeyboardButton(month[0], callback_data=calendar_callback.new("MONTH", year, 2 * i + 1, "!")),
                InlineKeyboardButton(month[1], callback_data=calendar_callback.new("MONTH", year, (i + 1) * 2, "!")))
        return keyboard

    def calendar_query_handler(self, bot: TeleBot, call: CallbackQuery, name: str, action: str, year: int, month: int,
                               day: int):
        """
        ...
        if action == "IGNORE":
            ...
            bot.edit_message_text(text=call.message.text, chat_id=call.message.chat.id,
                                  ...
                                  reply_markup=self.create_calendar(name=name, year=int(preview_month.year),
            ...
            bot.edit_message_text(text=call.message.text, chat_id=call.message.chat.id,
                                  message_id=call.message.message_id,
                                  reply_markup=self.create_calendar(name=name, year=int(next_month.year),
...
class CallbackData:
    ...
    def __init__(self, prefix, *parts, sep=":"):
        if not isinstance(prefix, str):
            raise TypeError(f"Префикс должен быть экземпляром str, а не {type(prefix).__name__}")
        if not prefix:
