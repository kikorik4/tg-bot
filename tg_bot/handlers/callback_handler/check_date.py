...
from tg_bot.handlers.search_handlers import commands
from tg_bot.keyboards.inline.inline_calendar.calendar_inline import CallbackData, Calendar, check_month_day
        ...
        with bot.retrieve_data(call.message.chat.id) as data:
            if 'checkInDate' in data:
                ...
                if int(select_date) > checkin:
                    ...
                    data['max_rate'] = 0
                    if data['sort'] == 'RATING':
                        bot.set_state(call.message.chat.id, UserInputState.min_rate)
                        bot.send_message(call.message.chat.id, f'Введите минимальный рейтинг: ')
                    else:
                        bot.send_message(call.message.chat.id, data)
                        search_hotel.print_data(call.message, data)
                else:
                    bot.send_message(call.message.chat.id, 'Дата выезда должна быть больше даты заезда! '
                                                           'Повторите выбор даты!')
                    commands.my_calendar(call.message, 'выезда')
            else:
